...
/**
 * v0.5.1 native-SBS renderer.
 *
 * RayNeo switches the glasses into SBS in hardware (Menu + Volume Up). Android
 * only has to feed a correctly laid out double-wide frame: LEFT EYE | RIGHT EYE.
 * This first visual build remains MONO SBS, so both eye regions receive the same
 * captured frame. Each eye is aspect-fitted to avoid stretching.
 */
public final class SbsGLView extends GLSurfaceView {
    ...
    private static final class MirrorRenderer implements Renderer, SurfaceTexture.OnFrameAvailableListener {
        ...
        @Override public void onDrawFrame(GL10 gl) {
            ...
            drawEye(0,leftWidth,viewHeight);
            drawEye(leftWidth,viewWidth-leftWidth,viewHeight);
        }

        private void drawEye(int eyeX,int eyeWidth,int eyeHeight){
