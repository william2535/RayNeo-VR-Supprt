...
/**
 * Full-screen SBS renderer. A MediaProjection VirtualDisplay feeds a
 * SurfaceTexture, and this view draws the same captured frame into the left and
 * right halves of the display. This is MONO SBS: both eyes receive the same image.
 */
public final class SbsGLView extends GLSurfaceView {
    ...
    private static final class MirrorRenderer implements Renderer, SurfaceTexture.OnFrameAvailableListener {
        ...
        private int program, textureId, aPos, aTex, uTexMatrix;
        ...
        MirrorRenderer(SbsGLView owner, int captureWidth, int captureHeight, CaptureSurfaceListener listener) {
            this.owner=owner; this.captureWidth=captureWidth; this.captureHeight=captureHeight; this.listener=listener;
        ...
        @Override public void onDrawFrame(GL10 gl) {
            ...
            if(frameAvailable){
                try { surfaceTexture.updateTexImage(); surfaceTexture.getTransformMatrix(texMatrix); } catch(Throwable ignored) {}
            ...
            GLES20.glViewport(0,0,leftWidth,viewHeight);
            GLES20.glDrawArrays(GLES20.GL_TRIANGLE_STRIP,0,4);
            GLES20.glViewport(leftWidth,0,viewWidth-leftWidth,viewHeight);
            GLES20.glDrawArrays(GLES20.GL_TRIANGLE_STRIP,0,4);
        }
        ...
        private static int compile(int type,String src){
            ...
            if(ok[0]==0){String log=GLES20.glGetShaderInfoLog(s);GLES20.glDeleteShader(s);throw new RuntimeException("Shader compile failed: "+log);}return s;
        }
        private static int link(String vs,String fs){
            ...
            GLES20.glAttachShader(p,v);GLES20.glAttachShader(p,f);GLES20.glLinkProgram(p);GLES20.glDeleteShader(v);GLES20.glDeleteShader(f);
            int[] ok=new int[1];GLES20.glGetProgramiv(p,GLES20.GL_LINK_STATUS,ok,0);
            if(ok[0]==0){String log=GLES20.glGetProgramInfoLog(p);GLES20.glDeleteProgram(p);throw new RuntimeException("Program link failed: "+log);}return p;
        }
    }
}
