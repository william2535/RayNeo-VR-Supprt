...
/** v0.6 native-SBS service with live stereo plane tuning. */
public class SbsMirrorService extends Service {
    ...
    private static final String CHANNEL="rayneo_sbs_v060";
    ...
    private float separationPx=0f,zoomPercent=100f,verticalTrimPx=0f;
    ...
    @Override public int onStartCommand(Intent intent,int flags,int startId){
        ...
        readStereo(intent);
        startForeground(NOTIFICATION_ID,notification("Starting v0.6 stereo SBS…"));
        int resultCode=intent.getIntExtra(EXTRA_RESULT_CODE,Activity.RESULT_CANCELED);
        Intent resultData=intent.getParcelableExtra(EXTRA_RESULT_DATA);
        if(resultCode!=Activity.RESULT_OK||resultData==null){
            broadcast("STEREO SBS ERROR • screen capture permission missing");
            stopSelf();
            return START_NOT_STICKY;
        }
    ...
    private void applyStereo(){
        SbsGLView v=glView;
        if(v!=null)v.setStereoSettings(separationPx,zoomPercent,verticalTrimPx,swapEyes);
    }
        ...
        try{
            ...
            if(external!=null){
                ...
                broadcast((nativeWideMode?"NATIVE SBS WIDE MODE DETECTED":"EXTERNAL DISPLAY DETECTED • switch glasses to SBS")+
                        " • "+outputName+" • "+outputWidth+"×"+outputHeight+" • "+stereoSummary("ready"));
            }else{
                ...
                broadcast((nativeWideMode?"WIDE DEFAULT DISPLAY DETECTED":"NO SEPARATE RAYNEO DISPLAY • FALLBACK")+
                        " • "+outputWidth+"×"+outputHeight+" • "+stereoSummary("ready"));
            }
        }catch(Throwable t){
            broadcast("STEREO SBS ERROR • "+t.getClass().getSimpleName()+": "+safe(t.getMessage()));
            cleanup(true);
            stopSelf();
        }
    }
    ...
    private Display findBestExternalDisplay(DisplayManager dm){
        ...
        for(Display d:dm.getDisplays()){
            ...
            try{
                Display.Mode m=d.getMode();int w=m.getPhysicalWidth(),h=m.getPhysicalHeight();
    ...
    private void createVirtualDisplay(Surface surface){
        ...
        try{
            virtualDisplay=projection.createVirtualDisplay(
                    "RayNeo-v0.6-Stereo-SBS",captureWidth,captureHeight,density,
            ...
            String msg=mode+" ACTIVE • "+route+" • output "+outputWidth+"×"+outputHeight+" • "+stereoSummary("STEREO");
            broadcast(msg);
            NotificationManager nm=(NotificationManager)getSystemService(NOTIFICATION_SERVICE);
            if(nm!=null)nm.notify(NOTIFICATION_ID,notification(msg));
        }catch(Throwable t){
            broadcast("STEREO SBS DISPLAY ERROR • "+t.getClass().getSimpleName()+": "+safe(t.getMessage()));
            stopMirror("Stereo SBS display failed");
        }
    }

    private String stereoSummary(String prefix){
        String kind=separationPx<=0.01f?"MONO L|R":String.format(java.util.Locale.UK,"%.0fpx EYE SEPARATION",separationPx);
        return prefix+" • "+kind+" • zoom "+Math.round(zoomPercent)+"% • V "+Math.round(verticalTrimPx)+"px"+(swapEyes?" • EYES SWAPPED":"");
    }

    private Notification notification(String text){
        Intent stop=new Intent(this,SbsMirrorService.class).setAction(ACTION_STOP);
        PendingIntent pi=PendingIntent.getService(this,62,stop,
                PendingIntent.FLAG_UPDATE_CURRENT|(Build.VERSION.SDK_INT>=23?PendingIntent.FLAG_IMMUTABLE:0));
        Notification.Builder b=Build.VERSION.SDK_INT>=26?new Notification.Builder(this,CHANNEL):new Notification.Builder(this);
        return b.setContentTitle("RayNeo v0.6 Stereo Depth Lab")
                .setContentText(text).setSmallIcon(android.R.drawable.ic_menu_view).setOngoing(true)
                .addAction(new Notification.Action.Builder(android.R.drawable.ic_menu_close_clear_cancel,"STOP SBS",pi).build()).build();
    }

    private void createChannel(){
        if(Build.VERSION.SDK_INT>=26){
            NotificationManager nm=(NotificationManager)getSystemService(NOTIFICATION_SERVICE);
            if(nm!=null)nm.createNotificationChannel(new NotificationChannel(CHANNEL,"RayNeo Stereo SBS",NotificationManager.IMPORTANCE_LOW));
        }
    }
